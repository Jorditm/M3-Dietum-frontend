    ...
    render() {
      return (
        <Consumer>
          ...
          {({ login, signup, user, logout, isLoggedin, showError }) => {
            return (
              <WrappedComponent
                login={login}
                signup={signup}
...
class AuthProvider extends React.Component {
  ...
  signup = (user) => {
    const { name, lastName, proName, email, password } = user;
  ...
  render() {
    ...
    const { isLoading, isLoggedin, user, showError } = this.state;
    ...
    return isLoading ? (
      ...
      <Provider value={{ isLoggedin, user, login, logout, signup, showError }}>
        {this.props.children}
      </Provider>
